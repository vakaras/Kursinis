...
\begin{itemize}
  ...
  \item gali reikėti pakeisti konkrečios klasės egzemplioriaus veikseną,
    nekeičiant jo klasės kodo (objektiniame, tai galima spręsti
    naudojant pavedimą vietoj paveldėjimo);
    TODO: UML pavyzdys sprendimo objektiniame panaudojant sąsajų hierarchiją,
    komponentinio pavyzdys (ar jis apskritai legalus, tai yra ar
    mastelis nėra per mažas?), Scala pavyzdys, AspectJ pavyzdys
    (\url{http://en.wikipedia.org/wiki/Aspect-oriented_programming}).
  \item dalykinės srities kodas „paslepiamas“ „palaikančio“ kodo
    (žurnalizavimas, transakcijos, saugumas ir t.t.). Šito komponentinis
    nesprendžia? (Žr.: „Mastering Enterprise JavaBeans“
